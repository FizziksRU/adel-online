...
// --- Шкала здоровья (планшет персонажа) ---
// Убивает ШЕСТАЯ рана: пять ран персонаж переживает. На шкале это шесть клеток
// (0 — целый, раны 1…5) плюс клетка черепа, на которую маркер уходит с шестой
// раной. Клетки из invLoss персонажа помечены значком инвентаря: на них
// теряется ячейка, и по шкале видно, до чего осталось сколько ран.
export const HEALTH_DEATH = 6;
export const HEALTH_CELLS = Array.from({ length: HEALTH_DEATH + 1 }, (_, i) => i);
export const HEALTH_ICON = { skull: '☠', marker: '🎲', invLoss: '🎒' };

// --- Проверка духа ---
// Проверка — отдельное действие игрока: движок ставит её в очередь, игрок
// бросает кубик ходом rollSpirit, результат видят все. Подписи причин нужны и
// плашке броска, и журналу.
export const SPIRIT_REASONS = {
  fire: 'Пожар',
  maneuver: 'Манёвр уклонения',
  panic: 'Паника',
  risky_special: 'Рискованное спец. действие',
};
// Та же причина в родительном падеже — для подписи под кубиком
// («Мэй Чжао против пожара: 4 > 2 — провал»).
export const SPIRIT_REASONS_OF = {
  fire: 'пожара',
  maneuver: 'манёвра уклонения',
  panic: 'паники',
  risky_special: 'рискованного действия',
};
// Расшифровка порога: игрок должен видеть, из чего он сложился, а не одно
// итоговое число. Ключи совпадают с источниками модификаторов в движке.
export const SPIRIT_MOD_NAMES = {
  teddy: 'плюшевый мишка',
  stress: 'стресс',
  explosions: 'взрывы',
};
// Грани кубика: анимация перебирает их подряд, потом останавливается на выпавшей.
